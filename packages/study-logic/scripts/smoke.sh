...
curl -sS -X POST "$BASE/notebooks/nb_bio/topics/propose"
echo

echo "== quizzes before confirm (expect 409 TopicsUnconfirmed)"
curl -sS -X POST "$BASE/notebooks/nb_bio/quizzes"
echo

echo "== confirm inferred topics"
curl -sS -X POST "$BASE/notebooks/nb_bio/topics/confirm"
echo

echo "== list topics"
curl -sS "$BASE/notebooks/nb_bio/topics"
echo

echo "== create pretest"
QUIZ_JSON="$(curl -sS -X POST "$BASE/notebooks/nb_bio/quizzes")"
echo "$QUIZ_JSON"
...
echo
echo "== grade attempt"
curl -sS -X POST "$BASE/quizzes/${QUIZ_ID}/attempts" \
  -H 'content-type: application/json' \
  -d "{\"item_id\":\"${ITEM_ID}\",\"selected_choice_id\":\"${CHOICE_ID}\"}"
echo

echo "== scoreboard"
curl -sS "$BASE/notebooks/nb_bio/scoreboard"
echo

echo "== empty vault (expect 422 InsufficientEvidence)"
curl -sS -X POST "$BASE/notebooks/nb_empty/topics/confirm" \
  -H 'content-type: application/json' \
  -d '{"names":["Mitosis"]}'
echo
curl -sS -X POST "$BASE/notebooks/nb_empty/quizzes"
echo
