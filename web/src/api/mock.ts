...
export class MockStudyApi implements StudyApi {
  ...
  async createSpecialists(notebookId: string, topicIds: string[]): Promise<CreateSpecialistsResult> {
    ...
    this.state.chats.push(chat);
    const text = `Focus chat for ${names.join(", ") || "selected topics"}. Shared scoreboard stays live. Close when you want a handoff back to the orchestrator.`;
  ...
  async sendChatMessage(chatId: string, input: SendChatMessageInput): Promise<SendChatMessageResult> {
    ...
    let text = (input.text ?? input.content ?? "").trim();
    ...
    const role = input.generate_quiz && !(input.text ?? input.content)?.trim() ? "assistant" : (input.role ?? "user");
    ...
    if (chat.kind === "specialist" && role === "user") {
      ...
      const reply = `Noted. This specialist stays scoped to ${names.join(", ") || "the selected topic"}. I will not invent a lecture here — close the chat to send a handoff to the orchestrator.`;
      this.state.messages.push({
        id: id("msg"),
        chat_id: chatId,
        role: "assistant",
