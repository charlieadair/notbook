  ...
  return (
    <div className="stack">
      ...
      {!board.loading && rows.length === 0 ? (
        <Banner>
          No topic scores yet — spawn offer stays empty until a pretest has attempts (Study-logic
          does not invent candidates). <Link to={`/notebooks/${notebookId}/quiz`}>Start pretest</Link>
        </Banner>
      ) : null}
